  ...
  return (
    <main className="container py-5">
      <div className="text-center mb-4">
        <h2 className="fw-bold text-white">Globale CO₂-Emissionen</h2>
        <p className="text-light">Vergleich der Emissionen nach Ländern und Unternehmen.</p>
      </div>
      ...
      <div className="table-responsive shadow rounded">
        <table className="table table-bordered table-hover align-middle">
          <thead className="table-primary" style={{ backgroundColor: "#284F8F", color: "#fff" }}>
            <tr>
              <th onClick={() => handleSort("label1")} style={{ cursor: "pointer" }}>
                {view === "countries" ? "Unternehmen" : view === "companies" ? "Land" : "Land"}
                {sortConfig.key === "label1" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th onClick={() => handleSort("label2")} style={{ cursor: "pointer" }}>
                {view === "countries" ? "Land" : view === "companies" ? "Unternehmen" : "Unternehmen"}
                {sortConfig.key === "label2" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th onClick={() => handleSort("emissions")} style={{ cursor: "pointer" }}>
                CO₂-Emissionen (t/Jahr)
                {sortConfig.key === "emissions" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
            </tr>
          </thead>
          <tbody>
